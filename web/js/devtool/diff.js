...
//
// 소유 범위
//   diffTabId, colorizeDiff: 레포·stage까지 포함한 탭 정체성과 patch 표시 규칙.
//
// 제공 API
...
//
// 유지 조건
//   탭 id에는 레포와 staged 여부가 함께 들어가야 한다. 중첩 레포의 같은 파일 응답이 섞이면 안 된다.
//   새 요청 때 patch를 null로 되돌리고, untracked/staged 값을 git.diff 요청에 그대로 싣는다.
//
// 영향 범위
...
function diffTabId(root, file, staged) { return "diff:" + (staged ? "s:" : "") + (root ? root + ":" : "") + file; }
export function openDiff(file, opts) {
  ...
  const id = diffTabId(root, file, opts.staged);
  let t = getTabs(sp).find((x) => x.id === id);
  if (!t) { t = addTab(sp, { id, kind: "diff", label: file.split("/").pop() + (opts.staged ? " ◦스테이지" : ""), path: file, root, rel: opts.rel || "", staged: !!opts.staged, untracked: !!opts.untracked, patch: null }); }
  ...
  send({ type: "git.diff", path: root || file, file, staged: !!opts.staged, untracked: !!opts.untracked });
}
...
export function renderDiffView(t) {
  ...
  const tag = t.untracked ? "새 파일" : (t.staged ? "스테이지됨" : "변경");
